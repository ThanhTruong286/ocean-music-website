...
const Profile = () => {
    ...
    const [loading, setLoading] = useState(true); // Biến loading để hiển thị trạng thái tải dữ liệu
    const [accessToken, setAccessToken] = useState(null); // Lưu accessToken
        ...
        const fetchUser = async () => {
            if (accessToken) {
                try {
                    ...
                    const data = await fetchSpotifyUser(accessToken); // Gọi API lấy dữ liệu người dùng
                    setUser(data); // Lưu dữ liệu người dùng vào state
                    setLoading(false); // Đặt loading false khi đã có dữ liệu
                } catch (error) {
                    setError('Failed to fetch user data'); // Set error nếu có lỗi
                                        ...
                                        <div className="d-flex flex-wrap align-items-center justify-content-between">
                                            <div className="d-flex flex-wrap align-items-center">
                                                <div className="profile-img position-relative me-3 mb-3 mb-lg-0 profile-logo profile-logo1">
                                                    <img src={user.images && user.images.length > 0 ? user.images[0].url : 'default-avatar.jpg'} alt="User-Profile" className="theme-color-default-img img-fluid rounded-pill avatar-100" loading="lazy" />
                                                </div>
                                                <div className="d-flex flex-wrap align-items-center mb-3 mb-sm-0">
                                                    <h4 id="fullname" className="me-2 h4">{user.display_name}</h4>
                                                </div>
                                            </div>
                            ...
                            <div className="col-lg-3">
                                <div className="card">
                                    ...
                                    <div className="card-body">
                                        ...
                                        <div className="mb-1">Email: <a style={{ fontSize: "20px" }} href="#" className="ms-3">{user.email}</a></div>
                                        <div className="mb-1">Loại tài khoản: <a style={{ fontSize: "20px", textTransform: "capitalize" }} href="#" className="ms-3">{user.product || "Thường"}</a></div>
                                        {/* <div>Followers: <span>{user.followers.total}</span></div> */}
                                        <hr />
                                        <ul className="d-flex nav nav-pills mb-0 text-center profile-tab nav-slider" data-toggle="slider-tab" id="profile-pills-tab" role="tablist">
                                            <li className="nav-item" role="presentation">
