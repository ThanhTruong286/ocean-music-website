...
import '../styles/artist.scss'; // Import file SCSS
import Header from "../components/Header";
import Sidebar from '../components/Sidebar';
import { fetchArtists } from '../api/api'; // Import hàm fetchArtists
...
const Artist = () => {
  ...
  useEffect(() => {
    const loadArtists = async () => {
      try {
        const data = await fetchArtists(); // Gọi API fetchArtists để lấy danh sách nghệ sĩ
        setArtists(data); // Cập nhật danh sách nghệ sĩ vào state
      } catch (err) {
        setError('Không thể tải danh sách nghệ sĩ');
        console.error(err); // In lỗi ra console nếu có
      } finally {
        setLoading(false); // Đặt trạng thái loading là false sau khi tải xong
      }
    };

    loadArtists(); // Gọi hàm lấy dữ liệu nghệ sĩ
  }, []); // useEffect chỉ chạy một lần khi component được render

  if (loading) {
    return <div>Đang tải dữ liệu nghệ sĩ...</div>; // Hiển thị loading khi đang tải
  }
          ...
          <div className="artist-page">
            <div className="artist-section">
              ...
              <Swiper
                ...
                slidesPerView={3}  // Hiển thị 3 nghệ sĩ mỗi lần
                navigation // Thêm điều khiển để trượt
                loop // Cho phép lặp lại (quay vòng)
                  ...
                  artists.map((artist, index) => (
                    <SwiperSlide key={index}>
                      <div className="artist-card">
                        <img
                          src={artist.imgSrc || faker}
                          alt={artist.name}
                          className="artist-image img-fluid rounded me-3 "
                        />
